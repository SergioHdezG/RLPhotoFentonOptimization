...
class PPOProblem(PPOProblemParallelBase):
    ...
    def __init__(self, environment, agent):
        ...
        src.perox_complete_model_glob_coord_opt.py which has an special signature for the step method receiving: action,
    ...
    def collect_batch(self, render, render_after, max_step_epi, skip_states, verbose, discriminator=None, expert_traj=None):
        ...
        # self.number_of_optimizer_experiences = len(obs) * obs[0].shape[0] * obs[0].shape[1]
        self.obs_batch = []
        self.actions_batch = []
        ...
        for (o, r, d, p) in zip(obs, reward, done, params):
            first_obs = True
            if o.shape[0] > 1:
                for i in range(1, o.shape[0]):
                    ...
                    if self.n_stack is not None and self.n_stack > 1:
                        if first_obs:
                            ...
                            for _ in range(self.n_stack):
                                obs_queue[0].append(zero_obs)
                                obs_next_queue[0].append(zero_obs)
                            obs_queue[0].append(o[i-1])
                            obs_next_queue[0].append(o[i-1])
                            first_obs = False
                    if d[i]:
                        first_obs = True

                    # self.opt_obs.append(obs_queue)
                    ...
                    _, obs_next_queue, _, _, _, _ = self.store_episode_experience([p[i]],
                                                                                    [d[i]],
                                                                                    [o[i]],
                                                                                    [o[i-1]],
                                                                                    obs_next_queue,
        ...
        self.opt_obs = self.obs_batch
        self.opt_act = self.actions_batch
        self.opt_act_prob = self.actions_probs_batch
        self.opt_reward = self.rewards_batch
        self.opt_mask = self.masks_batch
        self.opt_values = self.values_batch
        self.obs_batch = []
        self.actions_batch = []
        self.actions_probs_batch = []
        self.rewards_batch = []
        ...
        print("Full time optimizer process: ", dt.datetime.now() - time_opt)
        time_rl = dt.datetime.now()
        # Stacking inputs
        if self.n_stack is not None and self.n_stack > 1:
        ...
        print("Time RL: ", dt.datetime.now() - time_rl)
        ...
        self.agent.remember(self.obs_batch, self.actions_batch, self.actions_probs_batch, self.rewards_batch,
                            self.values_batch, self.masks_batch, self.opt_obs, self.opt_act,
                            self.opt_act_prob, self.opt_reward, self.opt_values, self.opt_mask)

        print("Full time RL process: ", dt.datetime.now() - time_rl)
