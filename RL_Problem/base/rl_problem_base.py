...
class RLProblemSuper:
    ...
    def __init__(self, environment, agent):
        ...
        self.save_base = agent.save_base
        self.save_name = agent.save_name
        self.save_each = agent.save_each
        self.save_if_better = agent.save_if_better
        ...
        if verbose == 1:
            ...
            if self.save_each is not None and (e + 1) % self.save_each == 0:
                ...
                if self._check_for_save(rew_mean):
                    self.agent._save_network(self.save_base + self.save_name + str(int(rew_mean)))
