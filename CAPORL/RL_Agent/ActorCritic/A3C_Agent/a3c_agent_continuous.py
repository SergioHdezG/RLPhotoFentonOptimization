...
from CAPORL.RL_Agent.ActorCritic.A3C_Agent.Networks.a3c_net_continuous import *
...
class Worker(object):
    def __init__(self, name, globalAC, sess, state_size, n_actions, environment, n_stack=1, img_input=False,
                 lr_actor=0.0001, lr_critic=0.001, n_steps_update=10, action_bound=None, saving_model_params=None,
        ...
        self.sess = sess
        self.AC = ACNet(name, self.sess, state_size, self.n_actions, stack=stack, img_input=img_input, lr_actor=lr_actor, lr_critic=lr_critic,
    ...
    def act(self, obs, obs_queue):
        ...
        if self.n_stack is not None and self.n_stack > 1:
            ...
            return self.AC.choose_action(np.array(obs).reshape(-1, self.state_size))
