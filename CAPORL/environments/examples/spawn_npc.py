...
def main():
    ...
    try:
        ...
        spawn_points = []
        sp_points = world.get_map().get_spawn_points()
        sp_point = random.choice(sp_points) if sp_points else carla.Transform()
        ...
        for i in range(35):
            ...
            x = stating_points[index][0]
            y = stating_points[index][1] + random.random()*5
            transform = carla.Transform(carla.Location(x=x, y=y, z=sp_point.location.z+0.5),
                                        carla.Rotation(pitch=0, yaw=stating_points[index][2], roll=0))
