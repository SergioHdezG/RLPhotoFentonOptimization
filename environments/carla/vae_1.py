...
def test(x_test, input_shape):
    encoder, decoder, vae = load_model('/home/serch/AIRL/saved_models/Carracing_vae/vae_encoder_32_64_128_128',
                                       '/home/serch/AIRL/saved_models/Carracing_vae/vae_decoder_32_64_128_128',
                                       input_shape)
    generate_and_save_images((encoder, decoder), latent_dim, x_test)

def train(x_train, x_test, epochs, batch_size, input_shape):
    encoder, decoder, vae = build_model(input_shape)
    ...
    save_name = '/home/serch/AIRL/saved_experts/Carracing_vae/beta2_encoder_32_64_128_128'
    ...
    save_name = '/home/serch/AIRL/saved_experts/Carracing_vae/beta2_decoder_32_64_128_128'
...
def main():
    # images = []
    # directory = '/home/shernandez/CARLA_0.9.7/PythonAPI/CarlaTutorial/_out_small'
    ...
    images = load('/home/serch/AIRL/expert_demonstrations/human_expert_CarRacing_images_full.npy')

    images = np.array(images)

    # preprocess CarRacing-v0 images
    images = np.array([cv2.resize(o[:-12,:], (96, 96)) for o in images])
    # flip_images = np.array([np.fliplr(o) for o in images])

    # images = np.concatenate((images, flip_images))
    # for im, flip_im in zip(images, flip_images):
    #     im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
    #     cv2.imshow("obs", im)
    #     flip_im = cv2.cvtColor(flip_im, cv2.COLOR_BGR2RGB)
    ...
    # test(x_test, input_shape)
    # train(x_train, x_test, epochs, batch_size, input_shape)
