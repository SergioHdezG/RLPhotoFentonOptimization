...
def build_model(input_shape=(128, 128, 3)):
    ...
    encoder, inputs_rgb, shape, z_log_var, z_mean = build_encoder(input_shape)
    inputs_seg = Input(shape=input_shape, name='encoder_input_seg')
    inputs_dep = Input(shape=input_shape, name='encoder_input_dep')
    encoder.summary()
    # plot_model(encoder, to_file='vae_cnn_encoder.png', show_shapes=True)

    decoder_rgb = build_decoder(input_shape, shape, 'rgb')
    decoder_seg = build_decoder(input_shape, shape, 'seg')
    decoder_dep = build_decoder(input_shape, shape, 'dep')
    decoder_rgb.summary()
    # plot_model(decoder, to_file='vae_cnn_decoder.png', show_shapes=True)

    # instantiate VAE model
    outputs_rgb = decoder_rgb(encoder(inputs_rgb)[2])
    outputs_seg = decoder_seg(encoder(inputs_rgb)[2])
    outputs_dep = decoder_dep(encoder(inputs_rgb)[2])

    vae = Model(inputs=[inputs_rgb, inputs_seg, inputs_dep], outputs=[outputs_rgb, outputs_seg, outputs_dep], name='vae')



    vae_loss_rgb = reconstruction_loss(input_shape, inputs_rgb, outputs_rgb, z_log_var, z_mean)
    vae_loss_seg = reconstruction_loss(input_shape, inputs_seg, outputs_seg, z_log_var, z_mean)
    vae_loss_dep = reconstruction_loss(input_shape, inputs_dep, outputs_dep, z_log_var, z_mean)

    vae.add_loss(vae_loss_rgb)
    vae.compile(optimizer='adam')
    vae.summary()

    return encoder, decoder_rgb, decoder_seg, decoder_dep, vae


def reconstruction_loss(input_shape, inputs, outputs, z_log_var, z_mean):
...
def build_encoder(input_shape):
    ...
    return encoder, inputs, shape, z_log_var, z_mean


def build_decoder(input_shape, shape, type=''):
    # build decoder model
    latent_inputs = Input(shape=(latent_dim,), name='z_sampling_'+type)
...
def load_model(encoder_path, decoder_path):
    encoder, decoder_rgb, decoder_seg, decoder_dep, vae = build_model()
    ...
    # load weights into new model
    decoder_rgb.load_weights(decoder_path + ".h5")
    decoder_seg.load_weights(decoder_path + ".h5")
    decoder_dep.load_weights(decoder_path + ".h5")
...
def train(x_train, x_test, epochs, batch_size):
    ...
    # vae.compile(optimizer='adam')
    # vae.summary()
    models_rgb = (encoder, decoder_rgb)
    models_seg = (encoder, decoder_seg)
    models_dep = (encoder, decoder_dep)

    # train the autoencoder
    vae.fit([x_train_rgb, x_train_seg, x_train_dep], None,
            epochs=epochs,
            batch_size=batch_size,
            shuffle=True,
            validation_data=([x_test_rgb, x_test_seg, x_test_dep], None))
    ...
    save_name = '/home/serch/TFM/IRL2/CAPORL/environments/carla/carla_decoder_rgb'
    model_json = decoder_rgb.to_json()
    with open(save_name + ".json", "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    decoder_rgb.save_weights(save_name + ".h5")

    save_name = '/home/serch/TFM/IRL2/CAPORL/environments/carla/carla_decoder_seg'
    model_json = decoder_seg.to_json()
    with open(save_name + ".json", "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    decoder_seg.save_weights(save_name + ".h5")

    save_name = '/home/serch/TFM/IRL2/CAPORL/environments/carla/carla_decoder_dep'
    model_json = decoder_dep.to_json()
    with open(save_name + ".json", "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    decoder_dep.save_weights(save_name + ".h5")
    ...
    generate_and_save_images(models_rgb, latent_dim, x_test_rgb)
    generate_and_save_images(models_seg, latent_dim, x_test_rgb)
    generate_and_save_images(models_dep, latent_dim, x_test_rgb)
...
def main():
    ...
    gc.collect()
    # test(x_test)
    train([x_train_rgb, x_train_seg, x_train_dep], [x_test_rgb, x_test_seg, x_test_dep], epochs, batch_size)
