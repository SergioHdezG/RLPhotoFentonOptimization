...
//					double r3 = kP4 * G * (FeTot - cOld[1] - cOld[2]);
			...
			{
				if (peroxErr)
					error += 3.0e5 * pow(cNew[0] - peroxConc[iPerox+peroxPos], 2.0);
				peroxPos++;
			}
			if ((owner[expTime] == 1) || (owner[expTime] == 2))
			{
				if (bactErr)
					error += 1.25e-4 * G * pow(log(ViabB) - log(bactConc[iBact+bactPos]), 2.0);
				bactPos++;
			}
...
//	printf("generar curvas sodis 3");
	...
	for (int i_exp = 0; i_exp < 7; i_exp++)
	{
		...
		for (int expTime = 1; expTime < bData; expTime++)
		{
			...
			error += pow(log(ViabB) - log(bactSodisConc[iBact+expTime]), 2.0);
