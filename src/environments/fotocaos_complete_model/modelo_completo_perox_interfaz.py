...
class ModeloCinetico(EnvInterfaz):
    def __init__(self, perox=True, bact=False, sodis=False, model_to_load=None, params_to_optimize=None,
        ...
        if params_to_optimize is not None:
            ...
            params_to_optimize = []
            if sodis:
                params_to_optimize.extend(self.sodis_pos)

            if perox:
                params_to_optimize.extend(self.perox_pos)

            if bact:
                params_to_optimize.extend(self.bact_pos )
    ...
    def run(self, params):
        ...
        error = []
        if self.perox_model:
            curvas_perox = np.copy(self.model.generarCurvasPerox(params, self.n_param))
            error_perox = curvas_perox[12][0]
            error.append(error_perox)

        if self.bact_model:
            curvas_bact = np.copy(self.model.generarCurvasBact(params, self.n_param))
            error_bact = curvas_bact[12][0]
            error.append(error_bact)
        if self.sodis_model:
            curvas_sodis = np.copy(self.model.generarCurvasSodis(params, self.n_param))
        ...
        # Adjuntamos el tiempo correpondiente a cada medida para cada experimento.
    ...
    def render_perox(self, error, curvas, param, rew, perox=False, bact=False, sodis=False):
        """
        :error: array of 1 to 3 positions. If perox = bact = sodis = True then [perox_data, bact_data, sodis_data],
                if perox = False and bact = sodis = True then [bact_data, sodis_data]
                if bact = False and perox = sodis = True then [perox_data, sodis_data]
    ...
    def render_bact(self, error, curvas, param, rew, perox=False, bact=False, sodis=False):
        """
        :error: array of 1 to 3 positions. If perox = bact = sodis = True then [perox_data, bact_data, sodis_data],
                if perox = False and bact = sodis = True then [bact_data, sodis_data]
                if bact = False and perox = sodis = True then [perox_data, sodis_data]
    ...
    def render_sodis(self, error, curvas, param, rew, perox=False, bact=False, sodis=False):
        """
        :error: array of 1 to 3 positions. If perox = bact = sodis = True then [perox_data, bact_data, sodis_data],
                if perox = False and bact = sodis = True then [bact_data, sodis_data]
                if bact = False and perox = sodis = True then [perox_data, sodis_data]
        ...
        for i in range(len(indexes)):
            ...
            if np.any(np.isnan(curv)) or np.any(np.isinf(curv)):
                ...
                for j in range(curv.shape[0]):
                    ...
                    aux_err = np.square(np.log10(curv[j]) - np.log10(exp[1][j]))
