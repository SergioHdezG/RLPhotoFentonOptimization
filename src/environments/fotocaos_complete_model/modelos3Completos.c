				...
				for(int subt = 0; subt < timeSteps; subt++)
				{
					...
					for(int i = NOBACTCOMPOUND; i < (NOBACTCOMPOUND+NOH*NSod); i++)
					{
						...
						else 
						{
							...
							if(i < (NOBACTCOMPOUND+NOH*NSod-NSod))
							{
								cNew[i]=cNew[i]+At*(kROH2*cOld[i+5]);
							}
						}
...
extern "C" double * generarCurvasSodis(double * param, int nParam, bool peroxErr, bool bactErr)
{
