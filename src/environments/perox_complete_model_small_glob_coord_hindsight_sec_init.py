...
class env(EnvInterfaz):
    ...
    def __init__(self, params_to_optimize=None, sodis=False, perox=False, bact=False, log_alpha_values_init=False, log_scale=False):
        np.random.seed()
        ran = np.random.rand()
        # print('random init: ', ran)
        self.max_epochs = 19
        self.iterations = 0
        self.global_iter = 0
        self.n_data_division = 60
        # self.obs_space = len(self.ExperimentalModel.A) + len(self.ExperimentalModel.B)
        ...
        # self.observation_space = np.zeros(int(self.Mperox.n_data/self.n_data_division)+12 + self.n_param + 1, )


        # Tamaño ajustado a los datos experimentales
        # obs_dim = self.MCinetico.n_param + 156 * self.perox + 255 * self.bact + 126 * self.sodis
        # Toma solo los valores coincidentes en tiempo con las medidas experiemntales
        # [self.curvas_perox_index, self.curvas_bact_index, self.curvas_sodis_index] = self.MCinetico.get_exp_index()
        ...
        obs_dim = self.n_param + 156 * 2 * self.perox + 255 * 2 * self.bact + 126 * self.sodis
    ...
    def reset(self, test=False):
        ...
        if self.perox:
            ...
            index += 1
            goal_data.append(self.MCinetico.ravel_perox_data)
        ...
        if self.bact:
            ...
            list_curvas[index] = curvas_aux
            index += 1
            goal_data.append(self.MCinetico.ravel_bact_data)
        ...
        if self.sodis:
            ...
            list_curvas[index] = curvas_aux
            goal_data.append(self.MCinetico.ravel_sodis_data)
        ...
        self._dyn_reward_sodis = self.reward_her
        self._dyn_reward_perox = self.reward_her
        self._dyn_reward_bact = self.reward_her
        ...
        return np.concatenate([self.params, curvas_state, goal_state])
    ...
    def _select_rand_init(self, pos_to_init, params, prob_random_init, random_index=None):
        if prob_random_init < 0.10 and len(self.best_params_list) > 0:
            ...
            if self.log_scale:
                ...
                for i in range(max_i):
                    if params[i] > 0:
                        # params[i] = np.log(np.abs(params[i]))
            ...
            for i in pos_to_init:
                if self.MCinetico.params_to_optimize[i] < 12:
                    if False:  #self.bact:
                        params[i] = np.random.uniform(0, 15)
                    else:
    ...
    def _act(self, action):
        ...
        if self.perox and self.bact and self.sodis:
            ...
            action[11] = np.clip(action[4], 1, 30)
            action[12] = np.clip(action[4], 1, 30)
            ...
            action[9] = np.clip(action[4], 1, 30)
        elif self.bact:
    ...
    def step(self, action, test=False):
        ...
        for c in goal_data:
            goal_state.extend(c)
        obs = np.concatenate([self.params, curvas_state, goal_state])
    ...
    def reward_her(self, error, distancia):
        return (1 / error) - 1.
    ...
    def _stop_criteria(self):
        ...
        if len(self.MCinetico.epi_traj_err) > 1:
            ...
            # self.error_diff_mean = self.error_diff_mean / (self._select_rand_init - 1) + error_diff / self.global_steps
    ...
    def run_sodis_optimizer(self, opt, maxiter=40, epochs=10):
        ...
        for i in range(epochs):
            self.reset()
            res = minimize(self.run_model_sodis, self.params_ant, method=opt, bounds=None,
    ...
    def run_model_sodis(self, params):
        ...
        curvas_data = self.MCinetico.model.generarCurvasSodis(params, self.action_space.n)
