...
class env(EnvInterfaz):
    ...
    def __init__(self, params_to_optimize=None, sodis=False, perox=False, bact=False, log_alpha_values_init=False, log_scale=False):
        ...
        self.optimization_values_queue_aux = []  # cola para depurar el optimizador, no influye en el

        # optimizador de estantería
        self.last_obs = None  # Store the last iteration observation
        self.last_step_optimization_state = None
        self.sqp_obs_queue = deque(maxlen=perox_sqp_globals.n_stack)
        self.fin_optimize_flag = False
        self.last_optimizer_err = -1000  # stores the las valid error from the sqp optimization process
        self.opt_iter = 0
    ...
    def reset(self, test=False, opt=False):
        if len(self.optimization_values_queue) < 1:
            self.iterations = 0
            self.stacked_count = 0
            self.err_epi_hist = deque(maxlen=5)
            # self.Mperox.epi_traj = [[], [], []]
            # self.Mperox.epi_traj_err = []
            ...
            self.last_step_optimization_state = np.zeros(perox_sqp_globals.state_size)
            ...
            # if np.any(np.isinf(self.params)) or np.any(np.isnan(self.params)):
            #     print()
            self.last_obs = np.concatenate([self.params, curvas_state])

            return np.concatenate([self.params, curvas_state])
        else:
            # for o in self.last_obs:
            #     if np.isnan(o):
            #         print('is nan observation')
    ...
    def _act(self, action):
        ...
        self.params_ant = self.params
        # params = self.params + np.clip(action, -1., 1.)
        if np.isnan(self.params[0]):
            print('nan params')

        if self.perox and self.bact and self.sodis:
            action[:11] = np.clip(action[:11], -20., 20.)
            ...
            action[:9] = np.clip(action[:9], -20., 20.)
            # action[4] = np.trunc(np.clip(action[4] + 15, 0., np.abs(action[4] + 15)))
            ...
            action[:4] = np.clip(action[:4], -20., 20.)
            # action[4] = np.trunc(np.clip(action[4]+15, 0., np.abs(action[4]+15)))
    ...
    def record_opt_experiences(self, opt, maxiter=40):
        self.reset(opt=True)
        self.opt_iter = 0
        self.fin_optimize_flag = False
        res = minimize(self.run_model, self.params_ant, method=opt, bounds=None, callback=self.opt_callback,
        ...
        error = []
        curvas = []
        for list in self.optimization_values_queue:
            params.append(list[0])
            reward.append(list[1])
            done.append(list[2])
            error.append(list[3])
            curvas.append(list[4])
        obs = np.array([np.concatenate([p, c]) for p, c in zip(params, curvas)])

        for i in range(len(error)):
            if np.isnan(error[i]) or np.isnan(np.sum(curvas[i])) or np.isnan(np.sum(params[i])):
                # print('change NaN by -1')
                obs[i] = np.where(np.isnan(obs[i]), -1, obs[i])
                obs[i] = np.clip(obs[i], self.action_space.low, self.action_space.high)
                if np.isnan(np.sum(params[i])):
                    params[i] = np.array([0. for x in params[i]])

        return obs, reward, done, None, params, error
    ...
    def step(self, action, test=False):
        ...
        if np.any(np.isnan(error)) or check_curvas or np.any(np.isnan(params)) or \
           np.any(np.isinf(error)) or np.any(np.isinf(params)):
            ...
            # obs[:self.MCinetico.n_param] = np.clip(obs[:self.MCinetico.n_param],
            #                                                   self.action_space.low, self.action_space.high)
            # if np.any(np.isnan(params)) or np.any(np.isinf(params)):
            #     params = np.array([0. for x in params])
    ...
    def run_model(self, params):
        ...
        if self.perox:
            new_params[self.MCinetico.perox_pos] = params
        elif self.bact:
            new_params[self.MCinetico.bact_pos] = params

        if self.perox:
            curvas_data = np.copy(self.MCinetico.model.generarCurvasPerox(new_params, self.action_space.n))
        elif self.bact:
            curvas_data = np.copy(self.MCinetico.model.generarCurvasBact(new_params, self.action_space.n))
        error = curvas_data[12][0]
        self.optimization_values_queue_aux.append([params, error])

        return error

    def opt_callback(self, params):
        ...
        if not self.fin_optimize_flag:
            ...
            if self.perox:
                ...
                curvas_data = np.copy(self.MCinetico.model.generarCurvasBact(new_params, self.action_space.n))
            error = curvas_data[12][0]
            # curvas_data = self.Mperox.model.generarCurvas(params, self.n_param)

            curvas = []
            for i in range(curvas_data.shape[0] - 1):
                curvas.append(np.array([self.MCinetico.curv_times, curvas_data[i]]).reshape((2, 7201)))
            curvas = np.array(curvas)

            if self.perox:
                curvas_aux = []
                for i in range(self.curvas_perox_index.shape[0]):
                    curvas_aux.extend(curvas[i][1][self.curvas_perox_index[i]])

            if self.bact:
                curvas_aux = []
                for i in range(self.curvas_bact_index.shape[0]):
                    curvas_aux.extend(curvas[i][1][self.curvas_bact_index[i]])

            curvas = curvas_aux

            done = self._done(error)
            # Por algún motivo apaarecen resultados que devuelven un error válido pero alguna de las curvas persenta
            # valores nulos y es necesaria la siguiente comprobación.

            if not np.isnan(error) and np.any(np.isnan(curvas)):
                # print('is nan curvas')
                done = True
                error = np.nan

            reward = self._reward([error], None)
            #     # if np.isclose(params[0], item[0][0], atol=1e-16)\
            #     #         and np.isclose(params[1], item[0][1], atol=1e-16) \
            #     #         and np.isclose(params[2], item[0][2], atol=1e-16):
            #     if params[0] == item[0][0] and params[1] == item[0][1] and params[2] == item[0][2]:
            #         [_, reward, done, error, curvas] = item
            # self.last_step_optimization_values = []
            # if np.isnan(params[0]):
            #     print('what????')
            if perox_sqp_globals.critic_nn is not None:
                obs = perox_sqp_globals.preprocess(self.last_step_optimization_state)
                self.sqp_obs_queue.append(obs)
                # value = perox_sqp_globals.critic_nn.predict(np.array([self.sqp_obs_queue]))[0]
            value = 0.0
            ...
            if done:
                # TODO: Volver a activar el flag despue de depurar. Con el simplex no es necesario ya que se detienen
                ## ante los Nan
                # self.fin_optimize_flag = True
                zero_obs = np.zeros(perox_sqp_globals.state_size)
                for i in range(perox_sqp_globals.n_stack):
                    self.sqp_obs_queue.append(zero_obs)
                self.last_step_optimization_state = zero_obs
                # value = [np.nan]
            else:
                self.last_step_optimization_state = np.concatenate([params, curvas])
                self.last_optimizer_err = error
                # if np.any(np.isnan(self.last_step_optimization_state)):
                #     print('is nan obs')

            self.optimization_values_queue.append([params, reward, done, error, curvas, value])
            self.optimization_values_queue_aux.append([["", "", ""],""])
